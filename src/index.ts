...
app.get('/company', async (req: Request, res: Response) => {
  ...
  const xCookieString = req.headers['x-cookie-string'] as string | undefined
  ...
  if (Array.isArray(xCookieString)) {
    return res.status(400).json({ status: 'error', message: 'Invalid x-cookie-string header format.' })
  }
  if (!xCookieString) {
    return res.status(400).json({ status: 'error', message: 'no x-cookie-string header was provided.' })
  }
  ...
  try {
    const response = await fetch(`https://www.linkedin.com/voyager/api/organization/companies?decorationId=com.linkedin.voyager.deco.organization.web.WebFullCompanyMain-28&q=universalName&universalName=${companyUniName}`, {
      headers: { ...ALL_HEADERS, "cookie": xCookieString.toString().replace(/\\"/g, '"') },
      "body": null,
      "method": "GET"
    })
    ...
    if (!response.ok) {
      ...
      return res.status(409).json({ status: 'error', message: "request failed. no cookie string", statusText: response.statusText, x: xCookieString.replace(/\\"/g, '"') })
    }
    ...
    if (!companyInfo) {
      return res.status(409).json({ status: 'error', message: "request failed. no company info" })
    }
...
app.get('/company/posts', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!response.ok) {
      console.error('req error: ', response.statusText)
      return res.status(409).json({ status: 'error', message: "request failed. check cookie string", statusText: response.statusText })
    }
    ...
    if (!companyPosts) {
      return res.status(409).json({ status: 'error', message: "request failed. no company posts" })
    }
...
app.get('/company/jobs', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!companyJobs) {
      return res.status(409).json({ status: 'error', message: "request failed." })
    }
...
app.get('/company/people', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!companyEployees) {
      return res.status(409).json({ status: 'error', message: "request failed. no company employees" })
    }
